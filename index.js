...
var randomSelect = function(allAttendees) {
  ...
  allAttendees.forEach(function(singleAttendee) {
    if (attendeeById[singleAttendee.id] && attendeeById[singleAttendee.id].contact &&
        attendeeById[singleAttendee.id].contact.email) {
...
request({
  ...
  events.forEach(function(singleEvent) {
    if (singleEvent.id === '554118960225415a1b9b8a75') {
      var subsetAttendees = randomSelect(singleEvent.links && singleEvent.links.attendees &&
                                         singleEvent.links.attendees.linkage);
